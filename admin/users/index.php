...
$results = null;
if ($type == '') {
    $sql = "SELECT * FROM ai_users WHERE source = 'web' ORDER BY created DESC ";
    $results = $db->query($sql);
} else {
    $sql = "SELECT * FROM ai_users WHERE user_type = $type AND source = 'web' ORDER BY created DESC";
    $results = $db->query($sql);
}
$items = $results->result();

// $total = $db->query("SELECT COUNT(*) as total FROM ai_users WHERE source = 'web'")->row()->total;
...
<div class="card p-3">
    ...
    <!-- <div class="py-2 text-center">
        ...
        ?>
            ...
            <a href="<?= admin_url('users/index.php?page=' . ($page + 1) . '&user_type=' . $type) ?>" class="btn btn-sm btn-primary">Next</a>
        <?php
        } else if ($page >= $links) {
        ?>
            <a href="<?= admin_url('users/index.php?page=' . ($page - 1)  . '&user_type=' . $type) ?>" class="btn btn-sm btn-primary">Previous</a>
